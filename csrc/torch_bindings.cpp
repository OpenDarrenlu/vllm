...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, ops) {
  ...
  ops.def(
      "silu_and_mul_fp8_quant(Tensor! result, Tensor input, Tensor scale) -> "
      "()");
  ops.impl("silu_and_mul_fp8_quant", torch::kCUDA, &silu_and_mul_fp8_quant);

  ops.def(
      "silu_and_mul_int8_quant(Tensor! result, Tensor input, Tensor! scale) -> "
      "()");
  ops.impl("silu_and_mul_int8_quant", torch::kCUDA, &silu_and_mul_int8_quant);
