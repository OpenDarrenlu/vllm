...
// Activation and gating(static_fp8_quant) kernel template.
template <typename scalar_t, scalar_t (*ACT_FN)(const scalar_t&),
          typename fp8_type>
__global__ void act_and_mul_static_fp8_quant_kernel(
  ...
  const int32_t vec_loop_end =
      round_to_previous_multiple_of(block_end, elems_per_128bit_load);
...
// Activation and gating(dynamic_fp8) kernel template.
template <typename scalar_t, scalar_t (*ACT_FN)(const scalar_t&),
          typename fp8_type>
__global__ void act_and_mul_dynamic_per_token_fp8_quant_kernel(
    fp8_type* __restrict__ out,          // [..., d]
    const scalar_t* __restrict__ input,  // [..., 2, d]
  ...
  // We don't expect the hidden dimension to exceed 32 bits so int32 should
  // be safe here.
  const int32_t tgt_elems_per_block = d;
  const int32_t elems_per_block =
      round_to_next_multiple_of(tgt_elems_per_block, elems_per_128bit_load);
  ...
  // 128-bit vectorized code
  const int32_t vec_loop_end =
      round_to_previous_multiple_of(d, elems_per_128bit_load);
  const int32_t vec_end_idx = vec_loop_end / elems_per_128bit_load;
  ...
  float tmp_max = 0.0f;
#pragma unroll
  for (int32_t vec_idx = threadIdx.x; vec_idx < vec_end_idx;
       vec_idx += blockDim.x) {
    const int4 x_128bit = VLLM_LDG(&x_128bit_ptr[vec_idx]);
    const int4 y_128bit = VLLM_LDG(&y_128bit_ptr[vec_idx]);
    using scalar_128bit_vec_t = std::array<scalar_t, elems_per_128bit_load>;
...
void silu_and_mul_static_fp8_quant(torch::Tensor& out,    // [..., d]
  ...
  LAUNCH_ACTIVATION_GATE_STATIC_FP8_QUANT_KERNEL(vllm::silu_kernel);
}

// Launch activation, gating, and quantize kernel.
#define LAUNCH_ACTIVATION_GATE_DYNAMIC_PER_TOKEN_FP8_QUANT_KERNEL(KERNEL)   \
  int d = input.size(-1) / 2;                                               \
  int64_t num_tokens = input.numel() / input.size(-1);                      \
...
void silu_and_mul_dynamic_fp8_quant(torch::Tensor& out,   // [..., d]
  ...
  TORCH_CHECK(input.dtype() == torch::kFloat16 ||
              input.dtype() == torch::kBFloat16);
  TORCH_CHECK(input.size(-1) % 2 == 0);
  LAUNCH_ACTIVATION_GATE_DYNAMIC_PER_TOKEN_FP8_QUANT_KERNEL(vllm::silu_kernel);
}
