...
// Activation and gating(fp8_quant) kernel template.
...
// Activation and gating(int8_qunat) kernel template.
template <typename scalar_t, scalar_t (*ACT_FN)(const scalar_t&)>
__global__ void act_and_mul_int8_quant_kernel(
    int8_t* __restrict__ out,            // [..., d]
    const scalar_t* __restrict__ input,  // [..., 2, d]
    float* __restrict__ scale, const int d) {
  const int32_t blocks_per_token = gridDim.y;
  ...
  const scalar_t* __restrict__ x_ptr = input + token_idx * 2 * d;
  const scalar_t* __restrict__ y_ptr = input + token_idx * 2 * d + d;
  int8_t* __restrict__ out_ptr = out + token_idx * d;
...
#pragma unroll
  for (int32_t vec_idx = vec_start_idx + threadIdx.x; vec_idx < vec_end_idx;
    ...
    const int4 y_128bit = VLLM_LDG(&y_128bit_ptr[vec_idx]);
    using scalar_128bit_vec_t = std::array<scalar_t, elems_per_128bit_load>;
    using scalar_64bit_vec_t = std::array<int8_t, elems_per_128bit_load>;
...
// Launch activation, gating, and fp8_quantize kernel.
...
// Launch activation, gating, and int8_quantize kernel.
#define LAUNCH_ACTIVATION_GATE_INT8_QUANT_KERNEL(KERNEL)                      \
  int d = input.size(-1) / 2;                                                 \
  int64_t num_tokens = input.numel() / input.size(-1);                        \
  dim3 grid(num_tokens, num_tokens > 16 ? num_tokens > 32 ? 1 : 2 : 4);       \
  dim3 block(std::min(d, 512));                                               \
  ...
  VLLM_DISPATCH_FLOATING_TYPES(                                               \
      input.scalar_type(), "act_and_mul_kernel", [&] {                        \
        VLLM_DISPATCH_INTEGRAL_TYPES(                                         \
            out.scalar_type(), "fused_add_rms_norm_kernel_int8_type", [&] {   \
              vllm::act_and_mul_int8_quant_kernel<scalar_t, KERNEL<scalar_t>> \
...
void silu_and_mul_int8_quant(torch::Tensor& out,    // [..., d]
                             torch::Tensor& input,  // [..., 2 * d]
                             torch::Tensor& scale) {
  ...
  TORCH_CHECK(input.dtype() == torch::kFloat16 ||
              input.dtype() == torch::kBFloat16);
  TORCH_CHECK(input.size(-1) % 2 == 0);
  LAUNCH_ACTIVATION_GATE_INT8_QUANT_KERNEL(vllm::silu_kernel);
}